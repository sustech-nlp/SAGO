...
class RWKUQADataset(QADataset):
    """Dataset class for RWKU QA evaluation data"""
    ...
    def __getitem__(self, idx):
        ...
        if self.level == 1:
            question = 'Please complete the blank in the following question.\nQuestion: {}\n'.format(question)
        elif self.level == 2:
            question = 'Please briefly answer the following question.\nQuestion: {}\n'.format(question)
        elif self.level == 3:
            if type == 'incontext learning':
                question = 'Please briefly answer the following question.\n{}\n'.format(question)
            else:
                question = 'Please briefly answer the following question.\nQuestion: {}\n'.format(question)
        else:
            raise ValueError("Unsupported level: {}".format(self.level))

        # Not identical to the official RWKU evaluation prompt, but should be close enough
        if not (self.level == 3 and type == 'affirmative suffix'):
            question += "Answer:"
